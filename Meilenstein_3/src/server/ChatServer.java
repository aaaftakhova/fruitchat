...
public class ChatServer implements Runnable {
	...
	public void changeRoomTo(String room, Account acc) {
		String oldRoom = acc.getRoom();
		if (room.equals(oldRoom)) return;
		int response = acc.setRoom(room, rooms);
		if (response == 0) {
			roomMessage("[server]: " + acc.getLogin() + " changed to " + room, oldRoom);
			if (gui != null) {
				gui.addLog("room change: user: " + acc.getLogin() + " :from: " + oldRoom + " :to: " + room);
	...
	public void publicMessage(String message) {
		if (gui != null && !message.startsWith("[server]:") && !message.startsWith("[admin]:"))
	...
	public void roomMessage(String message, String room) {
		if (gui != null && !message.startsWith("[server]:")) {
	...
	public void setAccOffline(String login) {
		for (Account account : accounts) {
			if (account.getLogin().equals(login)) {
				...
				//change to defaultRoom
				changeRoomTo(defaultRoomName, account);
	...
	public void deleteAccount (String login) {
		...
		try {
			pWriter = new PrintWriter(new FileWriter("./data/accounts.txt"));
			for (Account a : accounts) {
				pWriter.println(a.getLogin() + " " + a.getPassword());
			}
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
		...
		//inform gui
		if (gui != null) {
			gui.addLog("account " + login + " :removed");
			gui.FillBenutzerList(accounts);
		}
	...
	public void rehabilitate(String login) {
		// TODO Auto-generated method stub
		if (gui!=null) {
			gui.addLog("user: " + login + " :debanned");
		}
	}
}
